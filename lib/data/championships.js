...
export async function getChampionships() {
  ...
  return championshipRows.map(c => ({
    ...
    points: c.points, fl: c.fastestLapPoint, pole: c.polePoint, dropWorst: c.dropWorst,
...
export async function getPointsPresets() {
  ...
  return presetRows.map(p => ({ id: p.id, name: p.name, points: p.points, fl: p.fastestLapPoint, pole: p.polePoint, builtin: p.builtin }));
}
