...
export async function getRosterAndTeams() {
  ...
  const raceRows = await db.query.races.findMany({ with: { classifications: true, pointAdjustments: true } });
  ...
  for (const r of raceRows) {
    ...
    // outside the classification loop above — it counts even for a
    // pole-sitter who didn't finish (DNF/DNS/DSQ), unlike every other
    // points source here.
    if (r.poleDriverId && champ.polePoint) {
      if (r.countsDrivers) driverPts[r.poleDriverId] = (driverPts[r.poleDriverId] || 0) + champ.polePoint;
      if (r.countsConstructors) {
        const teamId = driverTeamMap[r.poleDriverId];
        if (teamId) teamPts[teamId] = (teamPts[teamId] || 0) + champ.polePoint;
      }
    }
    // Same idea for a DNF who still covered enough ground: a flat bonus,
    // independent of classification, that doesn't touch anyone else's points.
    if (r.minLapsForPoints != null && champ.minLapsPoint) {
      for (const c of r.classifications) {
        if (c.status === "dnf" && c.lapsCompleted != null && c.lapsCompleted >= r.minLapsForPoints) {
          if (r.countsDrivers) driverPts[c.entrantId] = (driverPts[c.entrantId] || 0) + champ.minLapsPoint;
          if (r.countsConstructors) {
            const teamId = driverTeamMap[c.entrantId];
            if (teamId) teamPts[teamId] = (teamPts[teamId] || 0) + champ.minLapsPoint;
          }
        }
      }
    }
    // Discretionary steward point rulings — a flat, signed correction with
    // no objective criterion (unlike pole/min-laps), so it's summed as-is
    // rather than checked against a championship-level bonus value. Applies
    // to a race regardless of when the ruling was made, including well after
    // the race settled (see app/actions/pointAdjustments.js).
    for (const a of r.pointAdjustments) {
      if (r.countsDrivers) driverPts[a.entrantId] = (driverPts[a.entrantId] || 0) + a.points;
      if (r.countsConstructors) {
        const teamId = driverTeamMap[a.entrantId];
        if (teamId) teamPts[teamId] = (teamPts[teamId] || 0) + a.points;
      }
    }
  }
