...
export function buildSettlementInput(payload) {
  ...
  const classifications = isSeason ? [] : race.drivers.map(id => {
    ...
    const lapsCompleted = r && r.st === "dnf" && r.laps !== "" && r.laps != null ? Number(r.laps) : undefined;
    return { entrantId: id, position: null, status: r ? r.st : "dns", reason: r ? r.why : undefined, lapsCompleted };
  });
