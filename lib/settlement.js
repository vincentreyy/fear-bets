...
export function computeSettlement({
  ...
  if (season) {
    ...
    const net = Math.round(totalPool * (1 - rakePct / 100));
    ...
    // Rounding each winner's payout independently can leave net a few CR
    // short of what actually gets paid out (e.g. splitting 100 CR seven ways
    // rounds to 98) — rather than that remainder silently vanishing, it
    // folds into the house's rake. Only applies when there's a winning pool
    // to round against; with no winner, netPool/rakeAmount stay exactly the
    // rake split as before (the "carries over" case, handled by the caller).
    const winnersPaid = winners.reduce((s, w) => s + w.amount, 0);
    ...
    return {
      ...
      totalPool,
      netPool: winPool ? winnersPaid : net,
      rakeAmount: winPool ? totalPool - winnersPaid : totalPool - net,
    };
  }
  ...
  const net = Math.round(settlingPool * (1 - rakePct / 100));
  ...
  // Rounding each winner's payout independently can leave net a few CR short
  // of what actually gets paid out — rather than that remainder silently
  // vanishing, it folds into the house's rake (see the season branch above
  // for the same treatment). Only applies when there's a winning pool to
  // round against.
  const winnersPaid = winners.reduce((s, w) => s + w.amount, 0);
  ...
  if (race.countsDrivers || race.countsConstructors) {
    const pointsByEntrant = {};
    for (const [i, entrantId] of order.entries()) {
      const base = pointsScale[i] || 0;
      const bonus = fastestLapEntrantId === entrantId ? (flBonus || 0) : 0;
      if (base || bonus) pointsByEntrant[entrantId] = (pointsByEntrant[entrantId] || 0) + base + bonus;
    }
    // Pole rewards the qualifying lap, not the race result, so it's added
    // outside the order-based loop above — it counts even for a pole-sitter
    // who didn't finish (DNF/DNS/DSQ), unlike every other points source here.
    if (poleDriverId && polePoint) {
      pointsByEntrant[poleDriverId] = (pointsByEntrant[poleDriverId] || 0) + polePoint;
    }

    // Same idea for a DNF who still covered enough ground: a flat bonus,
    // independent of `order`, that doesn't touch anyone else's position or
    // points. Only DNF qualifies — DSQ is a rule violation (not a distance
    // question) and DNS completed zero laps by definition.
    if (minLapsForPoints != null && minLapsPoint) {
      for (const c of classifications) {
        if (c.status === "dnf" && c.lapsCompleted != null && c.lapsCompleted >= minLapsForPoints) {
          pointsByEntrant[c.entrantId] = (pointsByEntrant[c.entrantId] || 0) + minLapsPoint;
        }
      }
    }

    const perEntrant = Object.entries(pointsByEntrant)
  ...
  return {
    ...
    totalPool,
    netPool: winPool ? winnersPaid : net,
    rakeAmount: winPool ? settlingPool - winnersPaid : settlingPool - net,
  };
}
