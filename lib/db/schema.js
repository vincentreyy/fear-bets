...
export const championships = pgTable("championships", {
  ...
  polePoint: integer("pole_point").notNull().default(0),
  minLapsPoint: integer("min_laps_point").notNull().default(0),
...
export const races = pgTable("races", {
  ...
  poleDriverId: text("pole_driver_id"),
  minLapsForPoints: integer("min_laps_for_points"), // DNF threshold for the flat min-laps bonus; unset = bonus doesn't apply to this race
...
export const classifications = pgTable("classifications", {
  ...
  reason: text("reason"),
  lapsCompleted: integer("laps_completed"), // only meaningful when status = dnf, for the min-laps points bonus
}, (t) => ({
  pk: primaryKey({ columns: [t.raceId, t.entrantId] }),
}));
...
// Discretionary steward point rulings — a signed, per-driver flat correction
// to championship points, independent of the settlement math entirely (see
// app/actions/pointAdjustments.js). Unlike every other race-scoring field,
// this is never gated on race.status: it doesn't touch bets, payouts, or the
// rake ledger, so it's safe to apply (or delete) even after a race settles —
// that's the entire reason it exists as its own append-only table instead of
// a mutation of `classifications`.
...
export const houseLedgerTypeEnum = pgEnum("house_ledger_type", [
  "rake", "deposit", "withdrawal", "manual_in", "manual_out", "correction_in", "correction_out",
]);

// One append-only ledger for the shared house wallet — auto-recorded for
// events that already move real cash (rake collected at settlement,
// deposits approved, withdrawals paid) plus manual entries an admin logs
// directly (see app/actions/finance.js). amount is signed: + is money in,
// - is money out. No backfill of pre-existing history — see the plan.
export const houseLedger = pgTable("house_ledger", {
  id: id(),
  type: houseLedgerTypeEnum("type").notNull(),
...
export const racesRelations = relations(races, ({ many }) => ({
  ...
  pointAdjustments: many(pointAdjustments),
}));
...
export const pointAdjustmentsRelations = relations(pointAdjustments, ({ one }) => ({
  race: one(races, { fields: [pointAdjustments.raceId], references: [races.id] }),
}));
