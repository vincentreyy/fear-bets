...
export function AdminChampionships({ S }) {
  ...
  const [pMinLaps, setPMinLaps] = useState(0);
    ...
    {tab === "points" && <div className="grid g2" style={{ gridTemplateColumns: "1fr 360px", alignItems: "start" }}>
      <div className="card">
        ...
        <div className="flex" style={{ gap: 16, marginTop: 20, paddingTop: 16, borderTop: "1px solid var(--hair)" }}>
          ...
          <div><div className="cap">DNF MIN-LAPS BONUS</div><div className="num" style={{ fontSize: 20, fontWeight: 600 }}>+{ch.minLaps} pt</div>
            <div className="cap" style={{ color: "var(--muted)" }}>DNF past each race's laps threshold</div></div>
      ...
      <div className="card">
        ...
        {S.pointsPresets.map(p => {
          const on = ch.points.join() === p.points.join() && ch.fl === p.fl && ch.pole === p.pole && ch.minLaps === p.minLaps;
          return <div key={p.id} className="card-flat"
            ...
            <div className="flex" style={{ justifyContent: "space-between", alignItems: "center" }}>
              ...
              <div className="flex" style={{ gap: 6, alignItems: "center" }}>
                {on ? <span className="badge b-lock">Active</span> : <button className="btn btn-ghost btn-xs" onClick={() => run(setPointsScale, { championshipId: ch.id, points: p.points, fastestLapPoint: p.fl, polePoint: p.pole, minLapsPoint: p.minLaps })}>Apply</button>}
                <button className="btn btn-ghost btn-xs" onClick={() => { setPRows(p.points.slice()); setPFl(p.fl); setPPole(p.pole); setPMinLaps(p.minLaps); setPName(p.builtin ? "" : p.name); }}>Edit</button>
                {!p.builtin && <button className="btn btn-ghost btn-xs" onClick={() => run(deletePointsPreset, { id: p.id })}>Delete</button>}
              </div>
            </div>
            <div className="cap num" style={{ color: "var(--muted)" }}>{p.points.join(" · ")}{p.fl ? " · FL +" + p.fl : ""}{p.pole ? " · Pole +" + p.pole : ""}{p.minLaps ? " · Min-laps +" + p.minLaps : ""}</div>
          </div>; })}
      </div>
      <div className="card">
        ...
        <label className="f">DNF MIN-LAPS BONUS</label>
        <div className="cap" style={{ color: "var(--muted)", marginBottom: 8 }}>Awarded to a DNF driver who still clears the laps threshold set on that race.</div>
        <div className="flex" style={{ gap: 8, marginBottom: 14 }}>
          {[0, 1, 2, 3].map(v => <button key={v} className={"btn btn-xs " + (pMinLaps === v ? "btn-y" : "btn-ghost")} onClick={() => setPMinLaps(v)}>{v === 0 ? "None" : "+" + v}</button>)}
        </div>
        <div className="card-flat" style={{ background: "var(--elev)", padding: "12px 14px", marginBottom: 14 }}>
          <div className="cap" style={{ marginBottom: 6 }}>PREVIEW</div>
          <div className="num" style={{ fontSize: 13 }}>{pRows.join(" · ")}{pFl ? " · FL +" + pFl : ""}{pPole ? " · Pole +" + pPole : ""}{pMinLaps ? " · Min-laps +" + pMinLaps : ""}</div>
        ...
        <div className="flex" style={{ gap: 8 }}>
          <button className="btn btn-2 btn-sm" style={{ flex: 1 }} disabled={!pName.trim()} onClick={() => { run(savePointsPreset, { name: pName.trim(), points: pRows, fastestLapPoint: pFl, polePoint: pPole, minLapsPoint: pMinLaps }); setPName(""); }}>Save preset</button>
          <button className="btn btn-y btn-sm" style={{ flex: 1 }} onClick={() => run(setPointsScale, { championshipId: ch.id, points: pRows, fastestLapPoint: pFl, polePoint: pPole, minLapsPoint: pMinLaps })}>Apply to {ch.name}</button>
        </div>
