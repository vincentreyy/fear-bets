...
import { AdminSettle } from "@/components/AdminScreens";

export default async function AdminSettlePage() {
  await requireAnyPermissionOrRedirect(["enter_results", "resolve_disputes"]);
  ...
  return (
    <ErrorBoundary>
      <EntrantProvider drivers={roster} teams={teams}>
        <AdminSettle S={{ races, championships, bets, roster }} />
      </EntrantProvider>
    </ErrorBoundary>
  );
}
